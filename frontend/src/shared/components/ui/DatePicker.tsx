...
export function DatePicker({
  ...
  // Popover content styling for theme - using semantic dark tokens for contrast
  const popoverContentClasses =
    theme === "dark"
      ? "bg-[#2d2820] border-white/15 backdrop-blur-[30px] text-[#f5f5f5]"
  ...
  const calendarClassNames = {
    ...
    day_outside: cn(theme === "dark" ? "text-[#7a7a7a]" : "text-[#b8a898]"),
    day_disabled: cn(
      theme === "dark"
        ? "text-[#8b7a6a] opacity-60"
        : "text-[#b8a898] opacity-50",
    ),
          ...
          >
            <span
              className={
                displayValue
                  ? ""
                  : `text-[14px] ${theme === "dark" ? "text-[#d4d4d4]" : "text-[#7a6b5a]"}`
              }
            >
              {displayValue || placeholder}
            </span>
            <CalendarIcon
              className={`h-4 w-4 ${theme === "dark" ? "text-[#d4d4d4]" : "text-[#7a6b5a]"}`}
            />
          </button>
        </PopoverTrigger>
        <PopoverContent
