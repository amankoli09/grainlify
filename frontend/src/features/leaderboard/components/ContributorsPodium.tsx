import { Medal, Trophy, Crown, Sparkles, TrendingUp, TrendingDown } from 'lucide-react';
...
export function ContributorsPodium({ topThree, isLoaded, actualCount }: ContributorsPodiumProps) {
  ...
  const placementOrder = [2, 1, 3].filter(r => r === 1 || (r === 2 && showSecond) || (r === 3 && showThird));
  const rankMap: Record<number, LeaderData> = {
    1: topThree[0],
    2: topThree[1],
    3: topThree[2],
  };
  ...
  return (
    <div
      ...
      role="group"
      aria-label="Top contributors podium"
    >
        ...
        return (
          ...
          >
            ...
            >
              ...
              <div className="relative">
                ...
                <div className="text-center">
                  <div
                    className={`text-[${rank === 1 ? '15' : '14'}px] font-bold mb-1 truncate max-w-[140px] transition-colors ${
                  ...
                  <div
                    className={`text-[${rank === 1 ? '26' : '20'}px] font-black text-[#c9983a] ${rank === 1 ? 'animate-number-glow' : ''}`}
                    aria-label={`Score: ${data.score} points`}
                  >
                    {data.score}
                  </div>
                  <div className={`text-[11px] transition-colors ${isDark ? 'text-[#d4d4d4]' : 'text-[#7a6b5a]'}`}>
                    pts
                  </div>
                </div>
              </div>
            ...
            <div className={`flex items-center justify-center gap-1.5 backdrop-blur-[20px] ${rankBadgeBg(rank)} ${rankBadgeClass(rank)}`}>
              ...
              <span className={`text-[${rank === 1 ? '18' : '16'}px] font-bold transition-colors ${
                isDark ? 'text-[#f5f5f5]' : 'text-[#2d2820]'
              }`}>
                #{rank}
              </span>
